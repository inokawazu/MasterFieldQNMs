function horexp(
    mfe::MasterFieldEquation, w::Number, q::Number; hororder::Integer = 20
  )
    w, q = promote(w, q)
    T = real(eltype(w))
    ...
    for ord in 2:hororder
        ...
        s = sum(
                (
                 bhor.coeffs[ord-(k-1)] + 
                 (k-1)*ahor.coeffs[ord-(k-1)] + 
                 (k-2)*(k-1)*chor.coeffs[ord-(k-1)]
                )*out_series[k] 
                for k in 1:ord-1
               )
        out_series[ord] = - s/(m*(m-1)*chor.coeffs[1] + m*ahor.coeffs[1] + bhor.coeffs[1])
    end
...
function hubeny_horowitz_qnm(
    mfe::MasterFieldEquation, w0::Number, q::Number; hororder::Integer = 20,
